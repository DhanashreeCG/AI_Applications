...
export function buildWorksheetContentPrompt(input: {
    ...
    count > 1
      ...
      : isUniversal
        ? [
            ...
            'Prefer activities[] as the semantic design (type, items, layoutIntent).',
            'content_html is optional; if present it must be a semantic HTML fragment only (no pixel layout CSS).',
            'Header fields stay plain text.',
          ].join(' ')
        : [
  ...
  return [
    ...
    ...(input.templateSlug === 'number_names' ? [
      ...
      '- Include between 3 and 6 pairs (min 3, max 6). Never exceed 6.',
      '- CRITICAL: Every left-column number must be UNIQUE. Never repeat a number to fill extra rows.',
      '- If the user gives a contiguous range whose size is 3–6 (e.g. "1-4" or "1 to 4"), emit EXACTLY that many pairs covering each value once (1,2,3,4 → 4 pairs). Do NOT pad to 6.',
      '- If the user gives a contiguous range larger than 6 (e.g. 1–20), pick 3–6 unique values from that range.',
      '- If the user lists specific numbers (3–6 of them), use exactly those values — one pair each, no padding.',
      '- If the user does not specify a number range, default to 1–6 (six pairs for numbers 1 through 6).',
      '- If the user asks for a broader/random set or numbers up to 20 without a tight range, pick 3–6 unique values from 0–20.',
    ...
    ...(isUniversal
      ? [
          'For universal_template — WORKSHEET COMPOSITION (design first, HTML second):',
          '',
          'Think in this order (DO NOT invent pixel layouts):',
          '  1) Understand age + learning objective from the user request.',
          '  2) Select meaningful learner interactions (genuinely different types).',
          '  3) Decide how much content each activity needs (items/images/labels).',
          '  4) Choose a SEMANTIC layout primitive per activity.',
          '  5) Emit activities[] JSON (+ optional content_html mirror).',
          'Never reason “I have 1104px so I will make four 250px divs.” The app composes the page.',
          '',
          'FIXED PAGE CHROME (already drawn — NEVER recreate):',
          '  • purple header = main_topic; blue banner = sub_topic; yellow footer signature',
          '',
          `CONTENT VIEWPORT: ${viewportW}×${viewportH}px — app allocates space & image sizes.`,
          ...(universalActivityPolicy
            ? buildUniversalActivityPolicyPromptLines(universalActivityPolicy)
            : []),
          '',
          'ACTIVITY COUNT ≠ CONTENT AMOUNT:',
          '  • Max activities = number of DIFFERENT learning interactions, not how sparse the page is.',
          '  • Age 2–3: ONE rich activity (large primary image + 2–3 choices / find-or-point), never one tiny icon on a blank page.',
          '  • Age 4–5+: prefer 3 strong activities over 4 weak ones.',
          '',
          'REQUIRED OUTPUT SHAPE:',
          '  • worksheet_type, main_topic, sub_topic, instruction_text, labels[], images[]',
          '  • activities[] — primary semantic design (preferred). Each activity:',
          '    { id, type, instruction, items[], leftItems?/rightItems? for match,',
          '      layoutIntent: { preferredLayout, density, imageImportance } }',
          '  • content_html — optional mirror using only semantic classes (no pixel CSS).',
          '  • activity type examples: recognize|match|circle|trace|find|choose|identify|classify|count|sequence|compare|complete|color|connect|label|point|odd-one-out',
          '  • preferredLayout examples: large-picture|image-row|image-grid|choice-grid|matching-columns|matching-grid|trace-row|classification-grid|single-focus|picture-and-word',
          '',
          'ONE QUESTION PER ACTIVITY (HARD RULE):',
          '  • Each activity has EXACTLY ONE short learner-facing question/instruction.',
          '  • Put it in activities[].instruction (or a single .ws-activity-instruction in HTML).',
          '  • Do NOT emit a separate activity title / section name.',
          '  • BAD: title "Find the Crab" + instruction "Point to the crab."',
          '  • BAD: "1. Match Two Pairs" / "Look and Name" / "Meet Sea Animals" as headings.',
          '  • GOOD: instruction only — "Point to the crab." / "Match each animal to its word."',
          '  • Keep instruction_text as the whole-page teacher line; do not repeat it inside each activity.',
          '',
          'ACTIVITY UNIQUENESS:',
          '  • BAD: Circle the fox / Circle the dog / Circle the cat (same interaction).',
          '  • BAD: Match animals to names / Match pictures to animal names (same interaction).',
          '  • GOOD: Point to the fox → Match each fox picture to its word → Trace the word fox.',
          '',
          'MATCHING:',
          '  • Use leftItems[] + rightItems[] (aligned pairs). Do NOT invent absolute connection lines.',
          '  • Prefer 2–3 pairs. App renders matching-columns with connector gutter.',
          '',
          'ASSET COMPATIBILITY:',
          '  • Never ask children to “color” a fully colored cartoon asset.',
          '  • Color activities ONLY with outline/line-art/black-and-white imageQuery.',
          '  • Otherwise choose find / circle / point / match / choose.',
          '',
          'IMAGES:',
          '  • images[i] describes {{IMAGE_(i+1)}}; use imageIndex on items (1-based).',
          '  • Optional role/importance/activityId. Never <img>, src=, filenames, or fixed width/height px.',
          '  • App sizes .ws-img-box from allocated space (1:1, object-fit contain).',
          '',
          'EDITABLE TEXT: wrap labels/trace words with data-editable="labels[N]" and return labels[].',
          'PUNCTUATION: !/? only in full sentences (5+ words).',
          'VISUAL: Toondemy font inherits; accents #6d28d9 #85cbf4 #fecd59 #67bd47 #f03a3e.',
          '  • worksheet_type: "universal_template"',
          '  • Do NOT emit layout catalog fields (layout_type/cards as template catalogs).',
          '  • Do NOT put Name/Date/signature/stars/main_topic/sub_topic inside activities.',
          '',
        ]
      : []),
    'Template metadata:',
