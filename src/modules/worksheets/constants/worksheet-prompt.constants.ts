  ...
  return [
    ...
    ...(isUniversal
      ? [
          ...
          '    <div class="ws-img-box" style="width:110px;height:110px;">{{IMAGE_1}}</div>',
          '  • Never use bare <img>, never src=, never filenames.',
          '  • Display size guide (pick by how many images share a row):',
          '      6 across a full row → box ~100–120px',
          '      4–5 across → ~130–160px',
          '      3 across / matching column → ~90–120px',
          ...
          '  B) Quad activities: 2×2 equal panels (tick grid | circle grid | match text↔picture | before/after match)',
          '  C) Fact cards + practice: 2×4 or 2×3 fact cards with numbered badges → bottom 50/50 (MCQ | dashed draw box)',
          '  Vary section count and interaction types when the topic changes. Never emit the same skeleton every time.',
          '',
          ...
          '  • Match with blue dots between columns (space for drawn lines)',
          '  • Short write lines / traceable dotted letters (use letter-spacing + dashed text decoration or underscore runs)',
          ...
          '  • Mid (7–8): short sentences, 3–4 sections, mix picture + simple writing',
          '  • Older (9–10): denser facts + practice, still child-friendly and playful',
          '  • Every section must teach the query objective (vocabulary, science fact, habit, phonics, etc.) — not decoration-only',
          ...
          '  • Section cards: border 2–3px soft color, border-radius 14–16px, padding 10–14px, background white or soft tint',
          '  • Consistent gutters 8–14px; equal column widths; align rows; balanced left/right',
          ...
          '  • Do NOT emit layout / layout_type / cards / items catalog fields — everything lives in content_html',
