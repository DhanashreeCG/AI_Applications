...
export class FlashcardsController {
  ...
  async renderAndNotify(
    ...
    const resources: { url: string; signedUrl: string; s3Key: string; folder: string; fileName: string }[] = [];
    ...
    for (let i = 0; i < cards.length; i++) {
      ...
      const fileName = result.fileName; // Uses exactly the slug generated by FlashcardPdfService
      const stored = await this.storageService.saveFile({
        requestId: '', // Ignored in storage path now, goes to root flashcards/
        fileName,
      ...
      resources.push({
        url: publicUrl, // Keep sending the whole public URL as requested
        signedUrl: stored.uri,
        s3Key: stored.path,
