...
import { EmbeddingMessage } from '../../../../common/interfaces/pipeline-messages.interface';
import { processPipelineJob } from './process-pipeline-job';
import { bullmqProcessorOptions } from './processor.constants';

@Processor(QUEUE_NAMES.embedding, bullmqProcessorOptions())
export class EmbeddingProcessor extends WorkerHost {
  private readonly logger = new StructuredLoggerService(EmbeddingProcessor.name);

  constructor(private readonly assetPipeline: AssetPipelineService) {
    super();
  }

  async process(job: Job<EmbeddingMessage>): Promise<void> {
